...
@section('preheader', __('Îți recomand călduros DateConta Facturare — facturi, proforme, e-Factura ANAF. Gratuit până la perioada de grație.'))
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f6f1e8;border-radius:12px;margin:0 0 22px 0;">
        <tr>
            <td style="padding:18px 20px;font-size:14px;line-height:1.6;color:#334e68;">
                <strong style="color:#0a3440;">{{ __('Ce poți face din prima zi:') }}</strong><br>
                ✓ {{ __('Facturi, proforme, avize, chitanțe') }}<br>
                ✓ {{ __('e-Factura ANAF și PDF profesional') }}<br>
                ✓ {{ __('Încasări, inclusiv plată cu cardul (cu contul tău de merchant)') }}<br>
                ✓ {{ __('Multi-firmă în același cont') }}
            </td>
        </tr>
    </table>
...
@endphp
    {{ __('Mesaj trimis de :sender — :company prin DateConta Facturare.', ['sender' => $senderName, 'company' => $companyName]) }}<br>
    {{ __('Contact:') }} <a href="mailto:{{ $contact }}" style="color:#ffd089;">{{ $contact }}</a>
@endsection

@section('legal')
    {{ __('Primești acest mesaj ca invitație la DateConta Facturare. Dacă nu e relevant, ignoră-l.') }}
@endsection
